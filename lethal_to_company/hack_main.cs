...
namespace lethal_to_company
{
  partial class hack : MonoBehaviour
  {
    ...
    public void OnGUI()
    {
      ...
      {
        GUI.Label(new Rect(100, 130 + (20 * index), 500, 30), $"{go.itemProperties.itemName} X: {go.transform.position.x}, Y: {go.transform.position.y}, Z: {go.transform.position.z}");
        index++;
      }
      index = 1;
      ...
      {
        GUI.Label(new Rect(810, 100 + (20 * index2), 1000, 30), $"{enemy.enemyType.enemyName} X: {enemy.transform.position.x}, Y: {enemy.transform.position.y}, Z: {enemy.transform.position.z}");
